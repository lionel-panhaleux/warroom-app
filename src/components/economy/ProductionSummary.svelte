<script lang="ts">
  ...
  let { available, spent, remaining, civilianGoods = $bindable(0), civGoodsPay = $bindable({ oil: 0, iron: 0, osr: 0 }), bombRepair = $bindable(false), onDone, allValid, nationId }: {
  ...
  let currentNationOk = $derived(affordable && civAssigned === civTotal)
  ...
  <div class="max-w-[480px] mx-auto px-3 py-2 space-y-1.5">
    ...
    <div class="flex gap-3 text-xs items-center">
      <span class="flex items-center gap-1">
        {@html icon('markers', 'civilian-goods', 'icon-xs')} Civ. Goods
      ...
      {#if !isChinaNoBomb}
        <label class="flex items-center gap-1">
          <input type="checkbox" bind:checked={bombRepair} />
          {@html icon('markers', 'wrench', 'icon-xs')} Bomb Repair (9{@html icon('resources', 'iron', 'icon-xs')})
        </label>
      {/if}
    </div>
