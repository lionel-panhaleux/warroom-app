...
export const ZONE_INFO: Record<HomelandZone, { name: string; effect: string }> = {
  ...
  Blue: { name: 'Unrest', effect: 'Pay 3 resources' },
  Yellow: { name: 'Dysfunctional', effect: 'Pay 3 + No rails, no sea trade, no port advantage' },
  Orange: { name: 'Disrupted', effect: 'Pay 3, no infrastructure + 3 fewer written orders' },
  Red: { name: 'Collapse', effect: 'Pay 3, -3 orders, no infrastructure + No new resource income' },
  Gray: { name: 'Desertion', effect: 'Pay 3, -3 orders, no infra , no income + Remove units' },
}
